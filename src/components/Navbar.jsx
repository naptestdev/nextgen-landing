...
export default function Navbar() {
  ...
  return (
    <div className="h-16 px-3 py-2 flex items-center justify-between">
      <div>
        <Link to="/" className="text-nav-logo text-2xl flex items-center gap-2">
          <svg
            className="w-6 h-6 fill-nav-logo"
      ...
      <div className="flex items-center gap-3 text-nav-link text-xl">
        <Link
          className={`${location.pathname === "/" ? "underline-nav-link" : ""}`}
          to="/"
        >
          Trang chủ
        </Link>
        <Link
          className={`${
            location.pathname === "/projects" ? "underline-nav-link" : ""
          }`}
          to="/projects"
        >
          Sản phẩm
